...
| ViewpointReview / ObjectReview | 436 / **6,748** |
| ThresholdSet / ClassDef / Run / RunBatch | 2 / 5 / **188** / **3** |

**교정 현황**: 삭제 5,465 · 되살림 634 · 분류 지정 1,250 · 코멘트 2
분류 내역: `round_frag` 598 · `rod_frag` 334 · `round` 159 · `rod` 82 · `eucampia` 77
...
DB 에만 쓰인다(`export_review.py` 가 아직 없다 — P02 5단계). **6,748 건이다.**
...
현재 12개 검사 전부 OK, 바인딩 `{'exact': 6748}` — 고아 교정 0.
...
| 4. 뷰어를 DB 로 | **끝** |
| 5. `export_review.py` | **남음** — 교정 6,748건이 DB 에만 있다 |
...
**지금 도는 판**: `IMAGE_TAG=v0.2.3` · `PIPELINE_TAG=v0.1.6`.
...
/srv/diatom/bin/smoke.sh v0.2.3
```
...
08-04 13:46 기준 통과 (`v0.2.3` · 슬라이드 10 · 교정 6,748).
...
3. **`export_review.py`** (P02 5단계) — 교정 6,748건의 두 번째 안전망
