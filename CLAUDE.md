...
```bash
python web/manage.py test viewer --exclude-tag browser   # 558개 · 7.5초
python web/manage.py test viewer                         # 691개 (브라우저 133개 포함, 181초)
```
...
- **사본을 손으로 지울 때 이름순으로 세우지 말 것** (2026-08-13). 이름순이
  시간순인 것은 **접두사가 한 가지일 때뿐이다.** `DiaRUGA_` 와 `diatom_`(048
  이전 이름)을 한 목록에 섞어 `sort` 하면 대문자가 앞이라 **최근 것이 앞으로
  가고**, `[:-keep]`·`head -n -10` 이 남기려던 바로 그것을 지운다 —
  `pre_deploy/` 의 최근 30개(`v0.6.3`~`v0.12.2`)를 그렇게 지웠다. 036 과 같은
  줄에서 이번엔 대소문자로 물렸다. **시각으로 정렬한다**(`ls -1t`).
  `ops/backup_db.py`·`sync_backup_nas.py` 는 안 물린다 — 앞엣것은 glob 이
  접두사를 못 박고, 뒤엣것은 **이름에서 시각을 못 읽는 파일을 남긴다**
