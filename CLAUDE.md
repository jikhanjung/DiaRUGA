...
```bash
python web/manage.py test viewer --exclude-tag browser   # 549개 · 7.2초
python web/manage.py test viewer                         # 679개 (브라우저 130개 포함, 175초)
```
...
- **저장을 거르는 갈래를 만들었으면 부르는 쪽을 전부 세어 본다** (116). `save()` 가
  "지나가는 판에는 저장하지 않는다"(074)로 조용히 돌아가는데, **검토 완료 버튼은
  그 전에 이미 상태를 바꾸고 칠하고 다음 시야로 넘어갔다** — 화면은 완료인데
  서버는 요청을 못 받았고, 실패 띠도 안 떴다(실패한 요청이 없으니까). 거르는
  조건은 *판에 앉는 판단*을 위한 것이었지 **시야에 붙는 표시**를 위한 것이
  아니었다. 실사용에서 시야 하나의 완료 표시가 그렇게 사라졌다
- **떠나기 전에 밀어낸 저장은 기다렸다 떠난다** (116). `leave()` 가 `flushSave()`
  의 약속을 안 기다리고 `location.href` 를 줬다 — keepalive 는 **본문 64 KB 를
  넘으면 보내 보지도 않고 거절한다**. 실패하면 **떠나지 않는다**: 떠나면 그
  판단이 화면에서도 사라져 실패 띠를 볼 사람이 없다
