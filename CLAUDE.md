...
- **사본을 손으로 지울 때, 파일 목록을 이름순으로 나열해 놓고 앞에서부터
  자르지 말 것** (2026-08-13). "이름순으로 나열하면 시간순이 된다" 가
  **접두사가 한 가지일 때만 성립한다.** `DiaRUGA_` 와 `diatom_`(048 이전
  이름)이 섞인 디렉토리를 `ls | sort` 로 나열하면 대문자가 앞이라 **최근
  ...
  시간대였고 이번엔 대소문자였다. **시각으로 나열한다**(`ls -1t`).
  `ops/backup_db.py`·`sync_backup_nas.py` 는 안 물린다 — 앞엣것은 glob 이
  접두사를 못 박아 한 가지 이름만 나열하고, 뒤엣것은 **이름에서 시각을 못 읽는
  파일을 남긴다**
