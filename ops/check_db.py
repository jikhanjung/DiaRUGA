...
def check_links(slug=None):
    ...
    # "학습 자료로 뽑을 때, 목록에 보일 때 이 판을 쓴다" 이므로(116) 그 자리에
...
def check_grade_pose(slug=None):
    ...
    **이것이 세 번째 자리다.** 화면이 파편에 칸을 안 보여주고(카탈로그 카드),
    ...
    묶을 때 값이 엇갈리면 거절하는 쪽이 나머지를 맡는다(108).
    ...
    **늘 0 인 검사는 두지 않는다** — 덮은 줄 알게 한다.
    ...
    objs = DiatomObject.objects.exclude(grade="", pose="")
    if slug:
    ...
    # **판이 모두 오검출인 개체.** "이 규조각은 오검출이면서 A 다" 가 되어 학습
    ...
    # 지키는 기준이지 제약이 아니다.
    n_a = objs.filter(grade="A", species="").count()
    if n_a:
