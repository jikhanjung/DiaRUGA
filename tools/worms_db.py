...
**덤프는 REST 를 대신하지 않는다.** 2026-07-01 판이라 그 뒤 갱신이 없고,
**규조가 11,526줄뿐이라 DiatomBase 전체보다 좁을 수 있다**(`--scope` 가 그것을
잰다). 그래서 여기서 **없다**는 것은 REST 에서 없다는 것보다 약한 말이다 —
근거로 쓸 때 어느 쪽에서 없었는지를 함께 적는다.
...
def against(out: Path, master: Path) -> None:
    ...
    **덤프에 없다고 판정을 뒤집지 않는다** — 해양 전용판이라 REST 가 확정한
    것의 38%만 담고 있다(`--scope`). 여기서 얻는 것은 **한쪽 방향뿐**이다:
