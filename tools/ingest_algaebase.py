...
# **`안 적혀 있다` 는 아무 말도 아니다** — 표의 그 칸이 비어 있었다는 뜻이라
# 어느 판정에도 못 이겨야 한다. `아직 안 찾았다`(사람이 "검색 안 함" 이라고
# **적은** 것)와 0 으로 같아 두면 비고 길이로 갈리는데, 그러면 잔여 작업지를
# 빈 칸으로 돌려받았을 때 그 줄이 뒤로 간다 (153 · 사용자 2026-08-25)
