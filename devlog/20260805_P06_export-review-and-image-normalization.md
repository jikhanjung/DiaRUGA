...
| 4. 파이프라인이 `image` 를 쓴다 | **끝** — 2026-08-05. 뷰어 v0.5.5 · 파이프라인 v0.2.1 |
| 5a. 조이기 | **끝** — 2026-08-05. `0022` · 뷰어 v0.5.6 · 파이프라인 v0.2.2 |
| 5b. 프레임별 검토 | 남음 — 결정은 §9 |
...
## 9. 5단계를 둘로 갈랐다 (사용자 결정 2026-08-05)

P06 은 5단계에 **조이기**와 **프레임별 검토**를 묶어 뒀는데 성격이 다르다.
조이기는 4단계가 끝나면 기계적으로 따라오고 결정이 필요 없다. 프레임별 검토는
결정 셋이 걸려 있고 사람의 품이 몇 배가 되는 기능이다.

**지금은 5a(조이기)까지 간다.** 5b 는 나중에 하되, 그때 쓸 결정은 이미 났다.

| 물음 | 결정 |
|---|---|
| "검토 완료" 의 단위 | **시야 단위 유지.** `ViewpointReview` 를 그대로 둔다 — 기존 432행·완료 343 이 그대로 살고 목록의 "검토 N/M" 도 안 바뀐다. 대신 "합성본만 보고 완료" 와 "프레임까지 보고 완료" 는 구분되지 않는다 |
| 교정을 저장할 수 있는 검출 | **현재 검출만** (지금 규칙 유지). YOLO 프레임 검출을 검토하려면 그것을 `is_current` 로 올려야 한다 — 한 시야에는 늘 한 엔진의 교정만 있다. 집계가 지금처럼 단순하게 남는다 |

**두 번째 결정이 5a 의 유일 제약을 바꾸지는 않는다.** "현재 검출만" 이어도
프레임별 YOLO 가 현재가 되면 한 시야에 현재 검출이 여럿(프레임마다 하나)이
되므로, 열쇠는 여전히 `(image, mask_key)` 여야 한다.

## 10. 4단계를 실제로 한 기록

```
0020  Image.viewpoint  CASCADE -> SET_NULL   프레임은 시야보다 오래 산다
0021  Image.stack      SET_NULL -> CASCADE   합성본·깊이맵은 묶음과 함께 죽는다
```

**둘의 방향이 반대인 것이 요점이다.** 시야 가르기(`regroup.apply_split`)는 시야를
지우고 다시 만드는데, **프레임은 원본 사진이라 살아남아 다른 시야로 묶이고**,
**합성본·깊이맵은 그 묶음에서 나온 것이라 무효가 된다** — 서로 다른 시야가 된
프레임들을 합쳐 놓은 그림이기 때문이다. 다시 합성하면 새로 생긴다.

사본에서 실제로 갈라 봤다: 이미지 1,952 → 1,950 (합성본·깊이맵 2개가 `Stack` 과
함께 빠짐), 프레임 3개는 살아남아 전부 새 시야로 옮겨 붙음, 찌꺼기 0.

만드는 문은 `web/viewer/images.py` 하나다 — 파이프라인 네 자리와 `regroup` 이
전부 여기를 지난다. 규칙이 흩어지면 한 곳만 옛 규칙으로 남는다(037~040 의 교훈).

### 굽는 데 8분 걸린 이유

`Dockerfile.pipeline` 은 코드 변경이 6 GB pip 층을 건드리지 않게 짜여 있는데
이번엔 안 먹었다. **base 이미지(`python:3.12-slim`)가 갱신되면서 그 위가 통째로
무효가 됐기 때문이다** — Dockerfile 도 requirements 도 안 바뀌었다.
`docker history` 의 층 나이로 확인했다(v0.2.0 은 pip 층이 41시간 전, v0.2.1 은
7분 전). 다음 빌드는 다시 캐시를 쓴다.

---

## 11. 5a — 조이기. 그리고 "조인 뒤에 돌려야 안다" 가 맞았다

나는 **조이기 전에 파이프라인을 한 바퀴 돌려 보자**고 했다. 사용자가 물었다 —
*"어차피 조이고 나서 돌려봐야 진짜 문제가 있는지 알 수 있는 거 아냐?"* 맞는
말이었다. 조이기 전에는 `ensure_image` 가 고장 나도 `image=NULL` 로 조용히
지나간다. **진짜 시험은 조인 뒤에만 존재한다.**

그래서 순서를 바꿨다: **조인 사본에 파이프라인 컨테이너를 붙여 검출을 돌린다.**
운영을 안 건드리면서 진짜 시험이 된다.
...
배포돼 있던 파이프라인 `v0.2.1` 은 아직 `target=` 을 쓰고 있었다 — 그 칸을 코드
에서 걷은 것이 그 이미지를 구운 **뒤**였기 때문이다. 조이기와 파이프라인 판은
**함께 나가야 한다.** 사본에서 났으니 운영은 멀쩡했다.

`v0.2.2` 로 다시 구워 같은 시험을 돌리니 조인 스키마에 검출 30개(개체 106)를
만들었고 `image` 가 빈 것은 0이었다.
...
flock /tmp/DiaRUGA-poll.lock          폴러를 세운다
  ...
  PIPELINE_TAG -> v0.2.2              **스키마보다 먼저.** 조여지는 순간
  deploy.sh v0.5.6                    옛 판은 INSERT 를 못 한다
```

### 함께 고친 것 — `save_review` 의 지우는 범위
...
다른 이미지 교정까지 쓸어 간다. 017·027·053 이 전부 이 줄에서 났다.
지금은 시야마다 이미지가 하나라 결과가 같다 — **같아 보일 때 고쳐 두는 것이
요점이다.**
...
| 자료 | 검출 2,076 · 교정 6,738 · 이미지 1,952 (그대로) |
| 검증 | `--verify` 아홉 항목 OK |
| 교정 | `export_review.py --check` 달라진 것 없음 |
| 화면 | 저장·되돌리기·읽기 전용·엔진 라디오 (사본에서 브라우저로) |
| 파이프라인 | v0.2.2 가 운영 DB 를 열고 조회 OK |
