...
`key` unique · `label` · `badge` · `color`(`"255,110,190"`) · `is_taxon` ·
`counted` · `sort_order` · `active`.
...
| 그 뒤에 더하거나 고칠 때 | 마이그레이션 | `0013_classdef_counted`(파편 둘을 `counted=False`), `0014_classdef_chaetoceros`(행 추가) |
...
`color` 는 표에 적혀 있지만 지금 화면의 색은 **CSS 가 키로 잡는다.** 그래서 분류를
하나 더하면 이 행 하나로 끝나지 않고 CSS 도 함께 가야 한다 — 표로 옮기다 만 자리다.
...
| 4 | 붙어 있는 `cls` 가 전부 `ClassDef` 에 있다 | 화면에 이름도 색도 없이 나온다 |
...
| `ObjectReview` / `ViewpointReview` | 6 752 / 436 |
| `Run` / `RunBatch` | 188 / 3 |
| `ThresholdSet` / `ClassDef` / `Setting` | 2 / 5 / 2 |

**교정 6 752건의 속내** — 삭제 5 466 · 되살림 637 · 분류 지정 1 253 · 메모 2.
바인딩은 전부 `exact` 다.
...
**분류 다섯** — 원형 · 원형 파편 · 봉상 · 봉상 파편 · Eucampia(`is_taxon`).
Chaetoceros 가 마이그레이션 0014 로 들어와 있고, 그것이 걸리면 여섯이 된다(§7).
...
`web/viewer/migrations/` 에 0001 … 0014. **DB 에 걸린 것은 0012 까지다**
(2026-08-04 기준). 0013·0014(`ClassDef.counted`, Chaetoceros)는 그것을 담은
이미지가 배포될 때 컨테이너 기동 시 `migrate` 로 걸린다.

**여기서 조심할 것:** 작업 트리의 `models.py` 는 새 칼럼을 아는데 DB 는 아직
모르는 상태가 정상적으로 존재한다. 그 사이에 **호스트에서 작업 트리 코드로 DB 를
물으면** `no such column` 으로 죽는다 — 실제로 이 문서를 쓰다가 그렇게 죽었다.
`dbrun.sh` 로 물으면 배포된 코드와 배포된 스키마가 함께 움직이므로 이 어긋남이
생기지 않는다. §1 의 "문은 하나" 가 이것을 막는다.
