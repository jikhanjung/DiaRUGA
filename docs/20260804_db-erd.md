...
테이블 16개와 그 사이의 관계 25개를 그린다. 각 칸의 뜻과 근거는
[DB 명세](20260804_db-specification.md)에 있고, 이 문서는 **무엇이 무엇에
매여 있는가**만 다룬다. 원본은 `web/viewer/models.py`.

도표만 크게 볼 것은 [ERD 한눈에](20260804_db-erd-poster.md) — 같은 원본에서
나오고 A3 가로로 뽑는다.
...
| `ObjectReview` | `(image, mask_key)` | **교정의 진짜 키** (P06 5a 에서 `viewpoint` → `image`) |
...
flowchart LR
    ...
    VP --> VR["ViewpointReview<br/>432<br/><small>완료 395</small>"]
    VP --> IM["Image<br/>1,952<br/><small>프레임 1,318 · 합성본 317 · 깊이맵 317</small>"]
...
- **`Viewpoint` 452 대 `Stack` 317** — 135개 시야는 사진이 한 장뿐이라(싱글턴)
  합성할 것이 없다. 그때는 그 한 장으로 검출을 돌린다.
- **`Image` 1,952 = 프레임 1,318 + 합성본 317 + 깊이맵 317.** 파일 하나에 행
  하나다. **깊이맵에는 검출이 붙지 않는다** — 종류가 스키마에 적혀 있다.
- **`Detection` 2,076 대 `Image` 1,952** — 한 이미지에 여러 묶음(SAM2·YOLO 3차)의
  검출이 쌓인다. `is_current` 는 **시야마다** 정확히 하나다.
- **`ObjectReview` 6,738** — 삭제 5,452 · 되살림 640 · 분류 지정 1,255 · 메모 2.
  바인딩은 전부 `exact`.
