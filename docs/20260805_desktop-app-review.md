...
| 길 | 대가 | 어울리는 경우 |
|---|---|---|
| **① 앱 전체를 AGPL-3.0 으로 공개** | 소스가 공개된다 | **학술 도구라면 오히려 자연스럽다.** 이 길이면 PyQt5(GPL v3)도 그대로 쓴다 |
| **② Ultralytics 상용 라이선스** | 비용 | 소스를 못 여는 사정이 있을 때 |
| **③ ultralytics 를 안 싣는다** | 다시 학습해야 한다 | 소스는 열기 싫고 돈도 안 쓸 때 |

③ 을 택하면 Apache 2.0 계열로 갈아탄다 — YOLOX · MMDetection(RTMDet-Ins) ·
detectron2 등. **가중치까지 AGPL 이 따라가는지는 다툼의 여지가 있다.**
ultralytics 로 학습한 `best.pt` 를 ONNX 로 내보내 중립 런타임에서 돌리면 코드
의존은 끊기지만, 배포 전에 확인이 필요하다 — 여기서 단정하지 않는다.

**이 결정이 먼저다.** ③ 이면 검출기를 다시 학습해야 하고, 그것은 데스크탑 앱보다
큰 일이다. ① 이면 지금 자산을 그대로 쓴다.
...
| 선택지 | 값 | 걸리는 것 |
|---|---|---|
| **PySide6** | 파이썬이라 `judge.py` 를 **그대로 쓴다** — 규칙이 두 벌이 되지 않는다. **LGPL 이라 배포가 자유롭다.** `devdocs/guides/desktop/` 이 정확히 이 스택 기준 | 새로 배우는 부분이 있다 |
| **PyQt5** | **Modan2 가 이미 23,000줄로 쓰고 있다** — 사내 경험과 빌드 자산이 그대로 | **GPL v3.** 4절에서 ① 을 택하면 문제없고, 아니면 걸린다 |
...
**PySide6 가 앞선다.** 이유는 성능이 아니라 둘이다 — **판정 규칙을 한 벌로 유지**
하는 것과, **LGPL 이라 4절의 선택을 열어 두는 것**. PyQt5 를 고르면 사실상 ①
(AGPL 공개)을 고른 것이 된다.
...
```mermaid
flowchart TB
    L["0단계 · 라이선스 결정<br/>AGPL 공개 / 상용 / 엔진 교체"] --> S1
...
**첫째, 4절(라이선스)을 먼저 정한다.** 이건 기술 결정이 아니라 정책 결정이고,
답에 따라 **검출기를 다시 학습해야 할 수도 있다.** 학술 도구라면 ①(AGPL 공개)이
자연스럽고 가장 싸다 — 지금 자산을 그대로 쓰면서 밖으로 낼 수 있다.
...
**단, 이 선택이 4절의 AGPL 을 불러온다** — 두 결정은 같이 내려야 한다.
